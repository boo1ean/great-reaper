...
Reaper.prototype.filter = function filter (filter) {
	this._filters.push(buildFilter(filter));
	return this;
};
...
function buildFilter (filter) {
	switch (true) {
		...
		case _.isObject(filter):
			return buildObjectFilter(filter);

		default: throw new Error('Filter must be object or function');
	}
}

function buildObjectFilter (filter) {
	return function filterObject (item) {
		return _.reduce(filter, applyPredicates, true);

		function applyPredicates (result, predicate, field) {
			return result && predicate(item[field]);
		}
	}
}

Reaper.prototype.transform = function transform (options) {
	this._transforms.push(buildTransform(options));
	return this;
};
...
function buildTransform (transform) {
	switch (true) {
		...
		default: throw new Error('Transform must be rules object or function');
	}
}
		...
		function finilizeProcessing (results) {
			...
			function filter () {
				if (self._filters.length > 0) {
					self._filters.forEach(function (itemFilter) {
						results = results.filter(itemFilter);
					});
				}
			}
		}
